...
| draft | `run_started` → `draft_delta`* (raw output-tool JSON fragments forwarded verbatim, exactly once, in order — the client concatenates; no server-side partial-JSON parsing; the opening name/id-only chunk's EMPTY arguments — `args` falsy, e.g. the `""` the gateway relays — must NOT stream: pydantic-ai's `args_as_json_str()` renders falsy args as `{}`, which would prepend a phantom fragment and break the concatenation contract) → `draft` (`OperationDraftEvent`: operation_id, state, full `DraftContent` flat; the validated atomic result) → `done` |
