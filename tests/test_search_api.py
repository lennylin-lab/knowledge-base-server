...
async def test_embedding_provider_from_settings_returns_none_without_key():
    assert embedding_provider_from_settings(Settings(OPENAI_API_KEY=SecretStr(""))) is None
    assert embedding_provider_from_settings(Settings(OPENAI_API_KEY=SecretStr("k"))) is not None


async def test_build_search_service_without_key_warns_vector_search_disabled():
    with capture_logs() as logs:
        build_search_service(Settings(OPENAI_API_KEY=SecretStr("")))
...
async def test_build_search_service_with_key_does_not_warn():
    with capture_logs() as logs:
        build_search_service(Settings(OPENAI_API_KEY=SecretStr("test-key")))
