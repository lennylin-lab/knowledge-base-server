...
>
> **v1.2 更新**：Gateway v1.2 已支持工具调用透传（含 `/v1/responses`、模型
> 能力发现与结构化输出），上述限制不再存在。工具调用的对接与验收见
> [`gateway-v1.2-integration.md`](./gateway-v1.2-integration.md)；本文其余
> 部分（安装、`.env`、key 获取、`KB_CHAT_*` 配置、错误表）仍然有效。
